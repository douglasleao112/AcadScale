...
import { motion } from 'transparent-ui';
...
>
      ...
      >
        <div className="relative w-full max-w-7xl h-[380px] md:h-[580px]">
          <img 
            src="https://i.ibb.co/nMqbfkgD/scale-rodape.png"   //https://i.imgur.com/svA6hmj.png
            alt="banner" 
        ...
        >
          <div className="min-h-[140px] md:min-h-[220px] flex flex-col justify-center">
            ...
            >
             ...
             <span className="font-black tracking-tighter text-[#22C55E] min-h-[1.1em] inline-block">
  {displayText || '\u00A0'}
</span>

                <span className="inline-block w-[3px] md:w-[4px] h-[0.8em] bg-[#22C55E] ml-1 md:ml-2 animate-pulse align-middle shadow-[0_0_15px_rgba(34,197,94,0.8)]" />
            </span>
            </h1>
          </div>
          ...
          <div className="flex flex-wrap justify-center items-center gap-x-6 md:gap-x-10 gap-y-1 mt-1 md:mt-2 h-8">
            {tags.map((tag, idx) => (
              <React.Fragment key={tag}>
                <button
                  ...
                  className={`text-[0.65rem] md:text-[0.7rem] tracking-[0.3em] md:tracking-[0.5em] font-bold uppercase transition-all duration-300 hover:text-white outline-none active:scale-95 ${
                    activeTags.includes(tag) 
                      ? 'text-[#7CFF9B] drop-shadow-[0_0_12px_rgba(124,255,155,0.75)]' 
                      : 'text-white/40'
                  }`}
                >
                  {tag}
                ...
                {idx < tags.length - 1 && (
                  <div className="w-1 h-1 md:w-1.5 md:h-1.5 bg-green-300/30 rounded-full" />
                )}
              </React.Fragment>
            ))}
          </div>
